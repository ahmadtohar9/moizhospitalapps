...
class AwalMedisTHTController extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('AwalMedisTHTModel');
    ...
    public function index()
    {
        ...
        $this->load->view('rekammedis/dokter/awalMedisTHT_view', $data);
    }
    ...
    public function get_history()
    {
        ...
        $history = $this->AwalMedisTHTModel->get_history($no_rkm_medis, $start_date, $end_date);
    ...
    public function get_detail()
    {
        ...
        $detail = $this->AwalMedisTHTModel->get_by_no_rawat($no_rawat);
        if ($detail) {
    ...
    public function save()
    {
        ...
        try {
            ...
            if (!empty($post['lokalis_image'])) {
                ...
                if ($data) {
                    $dir = FCPATH . 'assets/images/lokalis_tht/';
                    if (!is_dir($dir))
                        mkdir($dir, 0777, true);
            ...
            // Whitelist: only save columns that exist in penilaian_medis_ralan_tht table
            $allowed = [
                'no_rawat',
                'tanggal',
                'kd_dokter',
            ...
            if ($this->AwalMedisTHTModel->exists($post['no_rawat'])) {
                $this->AwalMedisTHTModel->update($post['no_rawat'], $post);
            } else {
                $this->AwalMedisTHTModel->insert($post);
            }
    ...
    public function delete()
    {
        ...
        $clean = str_replace('/', '', $no_rawat);
        $path = FCPATH . 'assets/images/lokalis_tht/lokalis_' . $clean . '.png';
        if (file_exists($path))
            unlink($path);

        if ($this->AwalMedisTHTModel->delete($no_rawat)) {
    ...
    public function print_pdf()
    {
        ...
        $data['asesment'] = $this->AwalMedisTHTModel->get_by_no_rawat($no_rawat);
        ...
        $html = $this->load->view('rekammedis/dokter/pdf_awal_medis_tht', $data, true);
        ...
        $mpdf->WriteHTML($html);
        $mpdf->Output('Asesmen_THT_' . $clean_no_rawat . '.pdf', 'I');
    }
}
