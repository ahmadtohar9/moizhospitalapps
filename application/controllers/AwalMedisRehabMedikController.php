...
class AwalMedisRehabMedikController extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('AwalMedisRehabMedikModel');
    ...
    public function index()
    {
        ...
        $this->load->view('rekammedis/dokter/awalMedisRehabMedik_view', $data);
    }
    ...
    public function get_history()
    {
        ...
        $history = $this->AwalMedisRehabMedikModel->get_history($no_rkm_medis, $start_date, $end_date);
    ...
    public function get_detail()
    {
        ...
        $detail = $this->AwalMedisRehabMedikModel->get_by_no_rawat($no_rawat);
        if ($detail) {
    ...
    public function save()
    {
        ...
        try {
            ...
            if (!empty($post['lokalis_image'])) {
                ...
                if ($data) {
                    $dir = FCPATH . 'assets/images/lokalis_rehab_medik/';
                    if (!is_dir($dir))
                        mkdir($dir, 0777, true);
            ...
            // Whitelist for penilaian_medis_ralan_rehab_medik
            $allowed = [
                'no_rawat',
                'tanggal',
                'kd_dokter',
            ...
            if ($this->AwalMedisRehabMedikModel->exists($post['no_rawat'])) {
                $this->AwalMedisRehabMedikModel->update($post['no_rawat'], $post);
            } else {
                $this->AwalMedisRehabMedikModel->insert($post);
            }
    ...
    public function delete()
    {
        ...
        $clean = str_replace('/', '', $no_rawat);
        $path = FCPATH . 'assets/images/lokalis_rehab_medik/lokalis_' . $clean . '.png';
        if (file_exists($path))
            unlink($path);

        if ($this->AwalMedisRehabMedikModel->delete($no_rawat)) {
    ...
    public function print_pdf()
    {
        ...
        $data['asesment'] = $this->AwalMedisRehabMedikModel->get_by_no_rawat($no_rawat);

        $clean_no_rawat = str_replace('/', '', $no_rawat);
        $lokalis_path = FCPATH . 'assets/images/lokalis_rehab_medik/lokalis_' . $clean_no_rawat . '.png';
        if (file_exists($lokalis_path)) {
            $data['lokalis_path'] = $lokalis_path;
        } else {
            $data['lokalis_path'] = FCPATH . 'assets/images/human_body_anatomy_rehab_medik.png';
        }

        $this->load->model('SettingModel');
        $data['setting'] = $this->SettingModel->get_setting();

        $html = $this->load->view('rekammedis/dokter/pdf_awal_medis_rehab_medik', $data, true);
        ...
        $mpdf->WriteHTML($html);
        $mpdf->Output('Asesmen_Rehab_Medik_' . $clean_no_rawat . '.pdf', 'I');
    }
}
