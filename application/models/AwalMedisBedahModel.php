...
class AwalMedisBedahModel extends CI_Model
{
    private $table = 'penilaian_medis_ralan_bedah';
    ...
    public function get_history($no_rkm_medis, $start_date = null, $end_date = null)
    {
        ...
        $this->db->join('reg_periksa r', 'p.no_rawat = r.no_rawat', 'left');
        $this->db->join('dokter d', 'p.kd_dokter = d.kd_dokter', 'left');
        $this->db->where('r.no_rkm_medis', $no_rkm_medis);

        if ($start_date) {
            $this->db->where('DATE(p.tanggal) >=', $start_date);
        }
        if ($end_date) {
            $this->db->where('DATE(p.tanggal) <=', $end_date);
        }
