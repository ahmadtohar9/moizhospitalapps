...
# Script untuk replace renderPenilaianAnak() dengan versi lengkap
# Usage: bash fix_anak_render.sh
...
echo "🔧 Fixing renderPenilaianAnak function..."
...
cat > /tmp/new_anak_function.js << 'EOF'
...
# Use Python to replace the function (more reliable than sed for multi-line)
...
# Read new function
with open('/tmp/new_anak_function.js', 'r') as f:
    new_function = f.read()

# Pattern to match the old function (from line 2435 to 2446)
...
echo ""
echo "✅ DONE! Function renderPenilaianAnak() has been updated!"
