<!-- ASESMEN AWAL MEDIS BEDAH - COMPLETE CLEAN VERSION -->
<style>
    .section-header {
...
<div class="container-fluid">
    <form id="formBedahAssessment" autocomplete="off">
        ...
        <div class="row">
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="status" value="" placeholder="Contoh: Stabil">
                </div>
            </div>
            <div class="col-md-3">
                <div class="form-group">
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <label>Genetalia</label>
                    <select class="form-control" name="genetalia">
                        <option value="">-- Pilih --</option>
                        <option value="Normal">Normal</option>
                        <option value="Abnormal">Abnormal</option>
                        <option value="Tidak Diperiksa">Tidak Diperiksa</option>
            ...
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <select class="form-control" name="muskulos">
                        <option value="">-- Pilih --</option>
                        <option value="Normal">Normal</option>
                        <option value="Abnormal">Abnormal</option>
                        <option value="Tidak Diperiksa">Tidak Diperiksa</option>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <div class="canvas-wrapper" style="height:450px;">
                        ...
                        <canvas id="bedahCanvas" width="600" height="350"></canvas>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="ket_lokalis" rows="18"
                        placeholder="Keterangan detail lokasi bedah yang diperiksa..."></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" name="pemeriksaan" rows="2" placeholder="EKG, USG, dll..."></textarea>
        </div>
...
<div class="container-fluid" style="margin-top:30px;">
    <div class="section-header" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%);"><i
            class="fa fa-history"></i> RIWAYAT ASESMEN BEDAH</div>
...
<script>
    ...
    var backgroundURL = '<?= base_url("assets/images/status_lokalis_bedah.png") ?>';

    setTimeout(function () {
        canvas = document.getElementById('bedahCanvas');
        if (!canvas) return;
    ...
    window.resetFormAndButton = function () {
        document.getElementById('formBedahAssessment').reset();
    ...
    // Form Submit
    document.getElementById('formBedahAssessment').addEventListener('submit', function (e) {
        e.preventDefault();
        const formData = new FormData(this);

        fetch('<?= base_url("AwalMedisBedahController/save") ?>', {
            method: 'POST',
            body: formData
        })
            .then(res => res.json())
    ...
    function loadHistory() {
        ...
        fetch('<?= base_url("AwalMedisBedahController/get_history") ?>?no_rkm_medis=' + noRkm + '&start_date=' + startDate + '&end_date=' + endDate)
            .then(res => res.json())
            .then(data => {
    ...
    function viewDetail(noRawat) {
        fetch('<?= base_url("AwalMedisBedahController/get_detail") ?>?no_rawat=' + noRawat)
            .then(res => res.json())
            .then(data => {
                if (data.status === 'success') {
                    const d = data.data;
                    ...
                    html += '<p><strong>BB:</strong> ' + (d.bb || '-') + ' kg</p></div></div>';

                    html += '<div style="background:#f8fafc; padding:15px; border-radius:8px; margin-bottom:15px; border-left:4px solid #8b5cf6;">';
                    html += '<h5 style="margin:0 0 10px 0; color:#8b5cf6;"><i class="fa fa-user-md"></i> III. PEMERIKSAAN SISTEMIK</h5>';
                    ...
                    html += '<p><strong>Genetalia:</strong> ' + (d.genetalia || '-') + '</p>';
                    html += '<p><strong>Columna:</strong> ' + (d.columna || '-') + '</p>';
                    html += '<p><strong>Muskuloskeletal:</strong> ' + (d.muskulos || '-') + '</p></div>';
                    ...
                    if (d.ket_lokalis) {
                        ...
                        var imgUrl = '<?= base_url("assets/images/lokalis_bedah/") ?>' + imgFile;
                    ...
                    if (d.lab || d.rad || d.pemeriksaan) {
                        ...
                        if (d.pemeriksaan) html += '<p><strong>Pemeriksaan Lain:</strong><br>' + d.pemeriksaan + '</p>';
                        html += '</div>';
                    }
                    ...
                    typeof Swal !== "undefined" && Swal.fire({
                        title: '<span style="color:#ec4899;"><i class="fa fa-file-medical"></i> Detail Asesmen Bedah</span>',
                        html: html,
                        width: '900px',
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    const fields = ['no_rawat', 'anamnesis', 'hubungan', 'keluhan_utama', 'rps', 'rpd', 'rpo', 'alergi',
                        'kesadaran', 'status', 'gcs', 'nyeri', 'td', 'nadi', 'rr', 'suhu', 'bb',
                        'kepala', 'thoraks', 'abdomen', 'ekstremitas', 'genetalia', 'columna', 'muskulos',
                        'lainnya', 'ket_lokalis', 'lab', 'rad', 'pemeriksaan', 'diagnosis', 'diagnosis2',
    ...
    function printSinglePDF(noRawat) {
        window.open('<?= base_url("AwalMedisBedahController/print_pdf?no_rawat=") ?>' + noRawat, '_blank');
    }
    ...
    function deleteAssessmentHistory(noRawat) {
        typeof Swal !== "undefined" && Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch('<?= base_url("AwalMedisBedahController/delete") ?>', {
                    method: 'POST',
    ...
    (function () {
        setTimeout(function () {
            const form = document.getElementById('formBedahAssessment');
            if (form) {
