<!-- ASESMEN AWAL MEDIS PARU (PEDIATRI) - COMPLETE CLEAN VERSION -->
<style>
    .section-header {
...
<div class="container-fluid">
    <form id="formParuAssessment" autocomplete="off">
        ...
        <!-- IV. STATUS LOKALIS PARU -->
        <div class="section-header"><i class="fa fa-image"></i> IV. STATUS LOKALIS PARU</div>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <div class="canvas-wrapper" style="height:450px;">
                        ...
                        <canvas id="paruCanvas" width="600" height="350"></canvas>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="ket_lokalis" rows="18"
                        placeholder="Keterangan detail lokasi paru yang diperiksa..."></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="pemeriksaan" rows="3"
...
<div class="container-fluid" style="margin-top:30px;">
    <div class="section-header" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%);"><i
            class="fa fa-history"></i> RIWAYAT ASESMEN PARU</div>
...
<script>
    ...
    var backgroundURL = '<?= base_url("assets/images/status_lokalis_paru.png") ?>';

    setTimeout(function () {
        canvas = document.getElementById('paruCanvas');
        if (!canvas) return;
    ...
    window.resetFormAndButton = function () {
        document.getElementById('formParuAssessment').reset();
    ...
    // Form Submit
    document.getElementById('formParuAssessment').addEventListener('submit', function (e) {
        e.preventDefault();
        const formData = new FormData(this);

        fetch('<?= base_url("AwalMedisParuController/save") ?>', {
            method: 'POST',
            body: formData
        })
            .then(res => res.json())
    ...
    function loadHistory() {
        ...
        fetch('<?= base_url("AwalMedisParuController/get_history") ?>?no_rkm_medis=' + noRkm + '&start_date=' + startDate + '&end_date=' + endDate)
            .then(res => res.json())
            .then(data => {
    ...
    function viewDetail(noRawat) {
        fetch('<?= base_url("AwalMedisParuController/get_detail") ?>?no_rawat=' + noRawat)
            .then(res => res.json())
            .then(data => {
                if (data.status === 'success') {
                    const d = data.data;
                    ...
                    if (d.lainnya) html += '<p><strong>Pemeriksaan Lainnya:</strong><br>' + d.lainnya + '</p>';
                    html += '</div>';
                    ...
                    if (d.ket_lokalis) {
                        ...
                        var imgUrl = '<?= base_url("assets/images/lokalis_paru/") ?>' + imgFile;
                    ...
                    typeof Swal !== "undefined" && Swal.fire({
                        title: '<span style="color:#ec4899;"><i class="fa fa-file-medical"></i> Detail Asesmen Paru</span>',
                        html: html,
                        width: '900px',
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    const fields = ['no_rawat', 'anamnesis', 'hubungan', 'keluhan_utama', 'rps', 'rpd', 'rpo', 'alergi',
                        'kesadaran', 'status', 'gcs', 'td', 'nadi', 'rr', 'suhu', 'bb', 'nyeri',
                        'kepala', 'thoraks', 'abdomen', 'muskulos', 'lainnya',
                        'ket_lokalis', 'lab', 'rad', 'pemeriksaan', 'diagnosis', 'diagnosis2', 'permasalahan', 'terapi', 'tindakan', 'edukasi'];
                    fields.forEach(f => {
    ...
    function printSinglePDF(noRawat) {
        window.open('<?= base_url("AwalMedisParuController/print_pdf?no_rawat=") ?>' + noRawat, '_blank');
    }
    ...
    function deleteAssessmentHistory(noRawat) {
        typeof Swal !== "undefined" && Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch('<?= base_url("AwalMedisParuController/delete") ?>', {
                    method: 'POST',
    ...
    (function () {
        setTimeout(function () {
            const form = document.getElementById('formParuAssessment');
            if (form) {
