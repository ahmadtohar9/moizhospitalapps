/* assets/js/soap_ralan.js (FINAL IMPROVED) */
(function ($, window, document) {
   ...
   * Endpoint namespace
   * ========================= */
  var API = (window.API && window.API.soapRalan) ? window.API.soapRalan : (window.API_URLS || {});
  if (!API || !API.getHasil) {
    console.error('❌ Endpoint SOAP Ralan tidak terdefinisi. Pastikan window.API.soapRalan (atau API_URLS legacy) ada di view.');
  }
  ...
  // Tambah ke semua POST secara otomatis
  $.ajaxSetup({
    ...
    function loadHasilSOAP() {
      $.get(API.getHasil, { no_rawat: noRawat, page: hasilPage, size: hasilSize }, function (response) {
        handleJSONorError(response, function (res) {
          ...
          if (typeof window.initHasilSoapDataTable === 'function') {
            setTimeout(function () {
              window.initHasilSoapDataTable();
            }, 100);
          }
