...
<head>
    <title>Asesmen Awal Medis GERIATRI</title>
    <style>
        body {
...
<body>
    ...
    <h3 style="text-align:center; margin:0 0 20px 0; text-decoration:underline;">ASESMEN AWAL MEDIS GERIATRI</h3>
    ...
    <table width="100%" cellpadding="3">
        ...
        <?php if (!empty($asesment['kondisi_umum'])): ?>
            <tr>
                <td colspan="4"><b>Kondisi Umum:</b> <?= $asesment['kondisi_umum'] ?></td>
            </tr>
        <?php endif; ?>
        <?php if (!empty($asesment['tulang_belakang'])): ?>
            <tr>
                <td colspan="4"><b>Tulang Belakang:</b> <?= $asesment['tulang_belakang'] ?></td>
            </tr>
        <?php endif; ?>
    </table>

    <div class="section-title">III. PENGKAJIAN KHUSUS GERIATRI</div>
    <table class="table-data">
        <tr>
            <th>Status Psikologis (GDS)</th>
            <td>: <?= $asesment['status_psikologis_gds'] ?></td>
        </tr>
        <tr>
            <th>Status Kognitif (MMSE)</th>
            <td>: <?= $asesment['status_kognitif_mmse'] ?></td>
        </tr>
        <tr>
            <th>Status Fungsional</th>
            <td>: <?= $asesment['status_fungsional'] ?></td>
        </tr>
        <tr>
            <th>Status Nutrisi</th>
            <td>: <?= $asesment['status_nutrisi'] ?></td>
        </tr>
        <tr>
            <th>Risiko Jatuh</th>
            <td>: <?= $asesment['skrining_jatuh'] ?></td>
        </tr>
        <tr>
            <th>Kondisi Sosial</th>
            <td>: <?= nl2br($asesment['kondisi_sosial']) ?></td>
        </tr>
    </table>

    <div class="section-title">IV. PEMERIKSAAN SISTEMIK & INTEGUMENT</div>
    <table width="100%" cellpadding="3">
        <tr>
            <td width="25%">
                <b>Kepala:</b> <?= $asesment['kepala'] ?? '-' ?><br>
                <?php if (!empty($asesment['keterangan_kepala'])): ?>
                    <small><i>ket: <?= $asesment['keterangan_kepala'] ?></i></small>
                <?php endif; ?>
            </td>
            <td width="25%">
                <b>Thoraks:</b> <?= $asesment['thoraks'] ?? '-' ?><br>
                <?php if (!empty($asesment['keterangan_thoraks'])): ?>
                    <small><i>ket: <?= $asesment['keterangan_thoraks'] ?></i></small>
                <?php endif; ?>
            </td>
            <td width="25%">
                <b>Abdomen:</b> <?= $asesment['abdomen'] ?? '-' ?><br>
                <?php if (!empty($asesment['keterangan_abdomen'])): ?>
                    <small><i>ket: <?= $asesment['keterangan_abdomen'] ?></i></small>
                <?php endif; ?>
            </td>
            <td width="25%">
                <b>Ekstremitas:</b> <?= $asesment['ekstremitas'] ?? '-' ?><br>
                <?php if (!empty($asesment['keterangan_ekstremitas'])): ?>
                    <small><i>ket: <?= $asesment['keterangan_ekstremitas'] ?></i></small>
                <?php endif; ?>
            </td>
        </tr>
    </table>
    ...
    <div style="margin-top:10px;">
        <b>Pemeriksaan Integument (Kulit):</b>
        <ul style="margin-top:5px; margin-bottom:5px;">
            <li>Kebersihan: <?= $asesment['Integument_kebersihan'] ?></li>
            <li>Warna: <?= $asesment['Integument_warna'] ?></li>
            <li>Kelembaban: <?= $asesment['Integument_kelembaban'] ?></li>
            <li>Gangguan: <?= $asesment['Integument_gangguan_kulit'] ?></li>
        </ul>
    </div>
