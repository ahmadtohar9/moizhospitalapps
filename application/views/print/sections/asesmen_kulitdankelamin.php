...
<div class="print-section">
    ...
    <?php if (!empty($d->kulitdankelamin->anamnesis)): ?>
    <div style="margin: 10px 0;">
        <strong>Anamnesis:</strong><br>
        <?= nl2br($d->kulitdankelamin->anamnesis) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->kulitdankelamin->pemeriksaan_fisik)): ?>
    <div style="margin: 10px 0;">
        <strong>Pemeriksaan Fisik:</strong><br>
        <?= nl2br($d->kulitdankelamin->pemeriksaan_fisik) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->kulitdankelamin->pemeriksaan_penunjang)): ?>
    <div style="margin: 10px 0;">
        <strong>Pemeriksaan Penunjang:</strong><br>
        <?= nl2br($d->kulitdankelamin->pemeriksaan_penunjang) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->kulitdankelamin->diagnosis)): ?>
    <div style="margin: 10px 0;">
        <strong>Diagnosis:</strong><br>
        <?= nl2br($d->kulitdankelamin->diagnosis) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->kulitdankelamin->tatalaksana)): ?>
    <div style="margin: 10px 0;">
        <strong>Tatalaksana:</strong><br>
        <?= nl2br($d->kulitdankelamin->tatalaksana) ?>
    </div>
    <?php endif; ?>
