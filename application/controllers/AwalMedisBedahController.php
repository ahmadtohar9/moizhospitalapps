...
class AwalMedisBedahController extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('AwalMedisBedahModel');
    ...
    public function index()
    {
        ...
        $this->load->view('rekammedis/dokter/awalMedisBedah_view', $data);
    }
    ...
    public function get_history()
    {
        ...
        $history = $this->AwalMedisBedahModel->get_history($no_rkm_medis, $start_date, $end_date);
    ...
    public function get_detail()
    {
        ...
        $detail = $this->AwalMedisBedahModel->get_by_no_rawat($no_rawat);
        if ($detail) {
    ...
    public function save()
    {
        ...
        try {
            ...
            if (!empty($post['lokalis_image'])) {
                ...
                if ($data) {
                    $dir = FCPATH . 'assets/images/lokalis_bedah/';
                    if (!is_dir($dir))
                        mkdir($dir, 0777, true);
            ...
            // Whitelist: only save columns that exist in penilaian_medis_ralan_bedah table
            $allowed = [
                'no_rawat',
                'tanggal',
                'kd_dokter',
            ...
            if ($this->AwalMedisBedahModel->exists($post['no_rawat'])) {
                $this->AwalMedisBedahModel->update($post['no_rawat'], $post);
            } else {
                $this->AwalMedisBedahModel->insert($post);
            }
    ...
    public function delete()
    {
        ...
        $clean = str_replace('/', '', $no_rawat);
        $path = FCPATH . 'assets/images/lokalis_bedah/lokalis_' . $clean . '.png';
        if (file_exists($path))
            unlink($path);

        if ($this->AwalMedisBedahModel->delete($no_rawat)) {
    ...
    public function print_pdf()
    {
        ...
        $data['asesment'] = $this->AwalMedisBedahModel->get_by_no_rawat($no_rawat);
        ...
        if (file_exists($lokalis_path)) {
            ...
            $data['lokalis_path'] = FCPATH . 'assets/images/human_body_anatomy_bedah.png';
        }

        $this->load->model('SettingModel');
        $data['setting'] = $this->SettingModel->get_setting();

        $html = $this->load->view('rekammedis/dokter/pdf_awal_medis_bedah', $data, true);
        ...
        $mpdf->WriteHTML($html);
        $mpdf->Output('Asesmen_Bedah_' . $clean_no_rawat . '.pdf', 'I');
    }
}
