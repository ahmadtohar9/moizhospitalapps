<?php if (!empty($d->bedahmulut)): ?>
<div class="print-section">
    <h3 style="background-color: #e3f2fd; padding: 8px; margin: 15px 0 10px 0; border-left: 4px solid #2196F3;">
        📋 ASESMEN MEDIS BedahMulut
    </h3>
    
    <table style="width: 100%; margin-bottom: 15px;">
        <tr>
            <td style="width: 25%; font-weight: bold;">Tanggal Pemeriksaan</td>
            <td>: <?= $d->bedahmulut->tanggal ?? '-' ?></td>
        </tr>
        <tr>
            <td style="font-weight: bold;">Dokter Pemeriksa</td>
            <td>: <?= $d->bedahmulut->nm_dokter ?? '-' ?></td>
        </tr>
    </table>
    
    <?php if (!empty($d->bedahmulut->anamnesis)): ?>
    <div style="margin: 10px 0;">
        <strong>Anamnesis:</strong><br>
        <?= nl2br($d->bedahmulut->anamnesis) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->bedahmulut->pemeriksaan_fisik)): ?>
    <div style="margin: 10px 0;">
        <strong>Pemeriksaan Fisik:</strong><br>
        <?= nl2br($d->bedahmulut->pemeriksaan_fisik) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->bedahmulut->pemeriksaan_penunjang)): ?>
    <div style="margin: 10px 0;">
        <strong>Pemeriksaan Penunjang:</strong><br>
        <?= nl2br($d->bedahmulut->pemeriksaan_penunjang) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->bedahmulut->diagnosis)): ?>
    <div style="margin: 10px 0;">
        <strong>Diagnosis:</strong><br>
        <?= nl2br($d->bedahmulut->diagnosis) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->bedahmulut->tatalaksana)): ?>
    <div style="margin: 10px 0;">
        <strong>Tatalaksana:</strong><br>
        <?= nl2br($d->bedahmulut->tatalaksana) ?>
    </div>
    <?php endif; ?>
    ...
    <div style="margin: 15px 0;">
        ...
        <img src="<?= base_url('assets/images/lokalis_bedahmulut/lokalis_' . $clean_no_rawat . '.png') ?>" 
