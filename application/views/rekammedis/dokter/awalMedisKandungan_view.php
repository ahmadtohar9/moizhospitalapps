<!-- ASESMEN AWAL MEDIS KANDUNGAN (OBSTETRI & GINEKOLOGI) - COMPLETE CLEAN VERSION -->
<style>
    .section-header {
...
<div class="container-fluid">
    <form id="formKandunganAssessment" autocomplete="off">
        ...
        <div class="section-header"><i class="fa fa-baby"></i> IV. PEMERIKSAAN OBSTETRI & GINEKOLOGI</div>

        <div class="row">
            <div class="col-md-2">
                <div class="form-group">
                    <label>TFU (cm)</label>
                    <input type="text" class="form-control" name="tfu" placeholder="Tinggi Fundus Uteri">
                </div>
            </div>
            <div class="col-md-2">
                <div class="form-group">
                    <label>TBJ (gram)</label>
                    <input type="text" class="form-control" name="tbj" placeholder="Taksiran Berat Janin">
                </div>
            </div>
            <div class="col-md-2">
                <div class="form-group">
                    <label>HIS</label>
                    <input type="text" class="form-control" name="his" placeholder="Kontraksi His">
                </div>
            </div>
            <div class="col-md-2">
                <div class="form-group">
                    <label>Kontraksi</label>
                    <select class="form-control" name="kontraksi">
                        <option value="">-- Pilih --</option>
            ...
            <div class="col-md-2">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="djj" placeholder="Denyut Jantung Janin">
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="inspeksi" rows="3" placeholder="Hasil inspeksi..."></textarea>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label>Inspekulo</label>
                    <textarea class="form-control" name="inspekulo" rows="3"
                        placeholder="Hasil inspekulo..."></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="vt" rows="3" placeholder="Hasil pemeriksaan VT..."></textarea>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label>RT (Rectal Toucher)</label>
                    <textarea class="form-control" name="rt" rows="3" placeholder="Hasil pemeriksaan RT..."></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label>USG (Ultrasonografi)</label>
                    <textarea class="form-control" name="ultra" rows="3" placeholder="Hasil USG..."></textarea>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label>Kardiotokografi (CTG)</label>
                    <textarea class="form-control" name="kardio" rows="3"
                        placeholder="Hasil kardiotokografi..."></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" name="konsul" rows="2" <></textarea>
        </div>
...
<div class="container-fluid" style="margin-top:30px;">
    <div class="section-header" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%);"><i
            class="fa fa-history"></i> RIWAYAT ASESMEN KANDUNGAN</div>
...
<script>
    ...
    window.resetFormAndButton = function () {
        document.getElementById('formKandunganAssessment').reset();
    ...
    // Form Submit
    document.getElementById('formKandunganAssessment').addEventListener('submit', function (e) {
        e.preventDefault();
        const formData = new FormData(this);

        fetch('<?= base_url("AwalMedisKandunganController/save") ?>', {
            method: 'POST',
            body: formData
        })
            .then(res => res.json())
    ...
    function loadHistory() {
        ...
        fetch('<?= base_url("AwalMedisKandunganController/get_history") ?>?no_rkm_medis=' + noRkm + '&start_date=' + startDate + '&end_date=' + endDate)
            .then(res => res.json())
            .then(data => {
    ...
    function viewDetail(noRawat) {
        fetch('<?= base_url("AwalMedisKandunganController/get_detail") ?>?no_rawat=' + noRawat)
            .then(res => res.json())
            .then(data => {
                if (data.status === 'success') {
                    const d = data.data;
                    ...
                    // IV. PEMERIKSAAN OBSTETRI & GINEKOLOGI
                    if (d.tfu || d.tbj || d.his || d.kontraksi || d.djj || d.inspeksi || d.inspekulo || d.vt || d.rt || d.ultra || d.kardio) {
                        html += '<div style=\"background:#f8fafc; padding:15px; border-radius:8px; margin-bottom:15px; border-left:4px solid #ec4899;\">';
                        html += '<h5 style=\"margin:0 0 10px 0; color:#ec4899;\"><i class=\"fa fa-baby\"></i> IV. PEMERIKSAAN OBSTETRI & GINEKOLOGI</h5>';
                        html += '<div style=\"display:grid; grid-template-columns:repeat(3, 1fr); gap:10px; font-size:13px;\">';
                        if (d.tfu) html += '<p><strong>TFU:</strong> ' + d.tfu + ' cm</p>';
                        if (d.tbj) html += '<p><strong>TBJ:</strong> ' + d.tbj + ' gram</p>';
                        if (d.his) html += '<p><strong>HIS:</strong> ' + d.his + '</p>';
                        if (d.kontraksi) html += '<p><strong>Kontraksi:</strong> ' + d.kontraksi + '</p>';
                        if (d.djj) html += '<p><strong>DJJ:</strong> ' + d.djj + ' x/mnt</p></div>';
                        if (d.inspeksi) html += '<p><strong>Inspeksi:</strong><br>' + d.inspeksi + '</p>';
                        if (d.inspekulo) html += '<p><strong>Inspekulo:</strong><br>' + d.inspekulo + '</p>';
                        if (d.vt) html += '<p><strong>VT:</strong><br>' + d.vt + '</p>';
                        if (d.rt) html += '<p><strong>RT:</strong><br>' + d.rt + '</p>';
                        if (d.ultra) html += '<p><strong>USG:</strong><br>' + d.ultra + '</p>';
                        if (d.kardio) html += '<p><strong>Kardiotokografi:</strong><br>' + d.kardio + '</p>';
                        html += '</div>';
                    }
                    ...
                    if (d.lab) {
                        ...
                        html += '<p><strong>Laboratorium:</strong><br>' + d.lab + '</p></div>';
                    }
                    ...
                    typeof Swal !== "undefined" && Swal.fire({
                        title: '<span style="color:#ec4899;"><i class="fa fa-file-medical"></i> Detail Asesmen Kandungan</span>',
                        html: html,
                        width: '900px',
                        showDenyButton: true,
                        ...
                        if (result.isDenied) {
                            window.open('<?= base_url("AwalMedisKandunganController/print_pdf?no_rawat=") ?>' + d.no_rawat, '_blank');
                        }
                    });
                }
            });
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    const fields = ['no_rawat', 'anamnesis', 'hubungan', 'keluhan_utama', 'rps', 'rpd', 'rpk', 'rpo', 'alergi',
                        ...
                        'ket_fisik', 'tfu', 'tbj', 'his', 'kontraksi', 'djj', 'inspeksi', 'inspekulo', 'vt', 'rt', 'ultra', 'kardio',
                        'lab', 'diagnosis', 'tata', 'konsul'];
                    fields.forEach(f => {
    ...
    function printSinglePDF(noRawat) {
        window.open('<?= base_url("AwalMedisKandunganController/print_pdf?no_rawat=") ?>' + noRawat, '_blank');
    }
    ...
    function deleteAssessmentHistory(noRawat) {
        typeof Swal !== "undefined" && Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch('<?= base_url("AwalMedisKandunganController/delete") ?>', {
                    method: 'POST',
    ...
    (function () {
        setTimeout(function () {
            const form = document.getElementById('formKandunganAssessment');
            if (form) {
