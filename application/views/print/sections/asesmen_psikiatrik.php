...
<div class="print-section">
    ...
    <?php if (!empty($d->psikiatrik->anamnesis)): ?>
    <div style="margin: 10px 0;">
        <strong>Anamnesis:</strong><br>
        <?= nl2br($d->psikiatrik->anamnesis) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->psikiatrik->pemeriksaan_fisik)): ?>
    <div style="margin: 10px 0;">
        <strong>Pemeriksaan Fisik:</strong><br>
        <?= nl2br($d->psikiatrik->pemeriksaan_fisik) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->psikiatrik->pemeriksaan_penunjang)): ?>
    <div style="margin: 10px 0;">
        <strong>Pemeriksaan Penunjang:</strong><br>
        <?= nl2br($d->psikiatrik->pemeriksaan_penunjang) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->psikiatrik->diagnosis)): ?>
    <div style="margin: 10px 0;">
        <strong>Diagnosis:</strong><br>
        <?= nl2br($d->psikiatrik->diagnosis) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->psikiatrik->tatalaksana)): ?>
    <div style="margin: 10px 0;">
        <strong>Tatalaksana:</strong><br>
        <?= nl2br($d->psikiatrik->tatalaksana) ?>
    </div>
    <?php endif; ?>
    ...
    $lokalis_path = FCPATH . 'assets/images/lokalis_psikiatrik/lokalis_' . $clean_no_rawat . '.png';
    if (file_exists($lokalis_path)):
    ?>
    <div style="margin: 15px 0;">
        <strong>Gambar Lokalis:</strong><br>
        <img src="<?= base_url('assets/images/lokalis_psikiatrik/lokalis_' . $clean_no_rawat . '.png') ?>" 
