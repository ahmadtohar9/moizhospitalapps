...
class AwalMedisNeurologiController extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('AwalMedisNeurologiModel');
    ...
    public function index()
    {
        ...
        $this->load->view('rekammedis/dokter/awalMedisNeurologi_view', $data);
    }
    ...
    public function get_history()
    {
        ...
        $history = $this->AwalMedisNeurologiModel->get_history($no_rkm_medis, $start_date, $end_date);
    ...
    public function get_detail()
    {
        ...
        $detail = $this->AwalMedisNeurologiModel->get_by_no_rawat($no_rawat);
        if ($detail) {
    ...
    public function save()
    {
        ...
        try {
            ...
            // Whitelist: only save columns that exist in penilaian_medis_ralan_neurologi table
            $allowed = [
                'no_rawat',
                'tanggal',
                'kd_dokter',
            ...
            if ($this->AwalMedisNeurologiModel->exists($post['no_rawat'])) {
                $this->AwalMedisNeurologiModel->update($post['no_rawat'], $post);
            } else {
                $this->AwalMedisNeurologiModel->insert($post);
            }
    ...
    public function delete()
    {
        ...
        if ($this->AwalMedisNeurologiModel->delete($no_rawat)) {
    ...
    public function print_pdf()
    {
        ...
        $data['asesment'] = $this->AwalMedisNeurologiModel->get_by_no_rawat($no_rawat);

        // No image needed for Neurologi
        $data['lokalis_path'] = null;

        $this->load->model('SettingModel');
        $data['setting'] = $this->SettingModel->get_setting();

        $html = $this->load->view('rekammedis/dokter/pdf_awal_medis_neurologi', $data, true);
        ...
        $mpdf->WriteHTML($html);
        $mpdf->Output('Asesmen_Neurologi_' . $clean_no_rawat . '.pdf', 'I');
    }
}
