...
class AwalMedisGeriatriController extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('AwalMedisGeriatriModel');
    ...
    public function index()
    {
        ...
        $this->load->view('rekammedis/dokter/awalMedisGeriatri_view', $data);
    }
    ...
    public function get_history()
    {
        ...
        $history = $this->AwalMedisGeriatriModel->get_history($no_rkm_medis, $start_date, $end_date);
    ...
    public function get_detail()
    {
        ...
        $detail = $this->AwalMedisGeriatriModel->get_by_no_rawat($no_rawat);
        if ($detail) {
    ...
    public function save()
    {
        ...
        try {
            ...
            if (!empty($post['lokalis_image'])) {
                ...
                if ($data) {
                    $dir = FCPATH . 'assets/images/lokalis_geriatri/';
                    if (!is_dir($dir))
                        mkdir($dir, 0777, true);
            ...
            // Whitelist: only save columns that exist in penilaian_medis_ralan_geriatri table
            // Asumsi tabel Geriatri sama dengan Urologi saat ini
            $allowed = [
                'no_rawat',
                'tanggal',
                'kd_dokter',
                ...
                'Integument_warna',
                'Integument_kelembaban',
                'Integument_gangguan_kulit',
                'status_fungsional',
                'skrining_jatuh',
                'status_nutrisi',
                'lainnya',
            ...
            // Filter post data against whitelist
            $post = array_intersect_key($post, array_flip($allowed));

            if ($this->AwalMedisGeriatriModel->exists($post['no_rawat'])) {
                $this->AwalMedisGeriatriModel->update($post['no_rawat'], $post);
            } else {
                $this->AwalMedisGeriatriModel->insert($post);
            }
    ...
    public function delete()
    {
        ...
        $clean = str_replace('/', '', $no_rawat);
        $path = FCPATH . 'assets/images/lokalis_geriatri/lokalis_' . $clean . '.png';
        if (file_exists($path))
            unlink($path);

        if ($this->AwalMedisGeriatriModel->delete($no_rawat)) {
    ...
    public function print_pdf()
    {
        ...
        $data['asesment'] = $this->AwalMedisGeriatriModel->get_by_no_rawat($no_rawat);

        $clean_no_rawat = str_replace('/', '', $no_rawat);
        $lokalis_path = FCPATH . 'assets/images/lokalis_geriatri/lokalis_' . $clean_no_rawat . '.png';
        if (file_exists($lokalis_path)) {
            $data['lokalis_path'] = $lokalis_path;
        } else {
            $data['lokalis_path'] = FCPATH . 'assets/images/human_body_anatomy_jantung.png';
        }

        $this->load->model('SettingModel');
        $data['setting'] = $this->SettingModel->get_setting();

        $html = $this->load->view('rekammedis/dokter/pdf_awal_medis_geriatri', $data, true);
        ...
        $mpdf->WriteHTML($html);
        $mpdf->Output('Asesmen_Geriatri_' . $clean_no_rawat . '.pdf', 'I');
    }
}
