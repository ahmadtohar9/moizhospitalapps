...
<head>
    <title>Asesmen Awal Medis UROLOGI</title>
    <style>
        body {
...
<body>
    ...
    <h3 style="text-align:center; margin:0 0 20px 0; text-decoration:underline;">ASESMEN AWAL MEDIS UROLOGI</h3>
    ...
    <table class="table-data">
        ...
        <?php if (!empty($asesment['riwayat_kebiasaan'])): ?>
            <tr>
                <th>Riwayat Kebiasaan</th>
                <td>: <?= nl2br($asesment['riwayat_kebiasaan']) ?></td>
            </tr>
        <?php endif; ?>
        <?php if (!empty($asesment['riwayat_operasi_urologi'])): ?>
            <tr>
                <th>Riwayat Operasi Urologi</th>
                <td>: <?= nl2br($asesment['riwayat_operasi_urologi']) ?></td>
            </tr>
        <?php endif; ?>
        <tr>
    ...
    <?php if (!empty($asesment['nyeri_ketok_cva']) || !empty($asesment['genitalia_eksternal']) || !empty($asesment['colok_dubur'])): ?>
        <table width="100%" cellpadding="3">
            <tr>
                <?php if (!empty($asesment['nyeri_ketok_cva'])): ?>
                    <td width="33%"><b>Nyeri Ketok CVA:</b> <?= $asesment['nyeri_ketok_cva'] ?></td>
                <?php endif; ?>
                <?php if (!empty($asesment['genitalia_eksternal'])): ?>
                    <td width="33%"><b>Genitalia Eksternal:</b> <?= $asesment['genitalia_eksternal'] ?></td>
                <?php endif; ?>
                <?php if (!empty($asesment['colok_dubur'])): ?>
                    <td width="34%"><b>Colok Dubur:</b> <?= $asesment['colok_dubur'] ?></td>
                <?php endif; ?>
            </tr>
        </table>
    <?php endif; ?>
    ...
    <table class="table-data">
        <?php if (!empty($asesment['urinalisis'])): ?>
            <tr>
                <th>Urinalisis</th>
                <td>: <?= nl2br($asesment['urinalisis']) ?></td>
            </tr>
        <?php endif; ?>
        <?php if (!empty($asesment['darah'])): ?>
            <tr>
                <th>Darah</th>
                <td>: <?= nl2br($asesment['darah']) ?></td>
            </tr>
        <?php endif; ?>
        <?php if (!empty($asesment['usg_urologi'])): ?>
            <tr>
                <th>USG Urologi</th>
                <td>: <?= nl2br($asesment['usg_urologi']) ?></td>
            </tr>
        <?php endif; ?>
        <?php if (!empty($asesment['radiologi'])): ?>
            <tr>
                <th>Radiologi</th>
