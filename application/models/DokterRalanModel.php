...
class DokterRalanModel extends CI_Model
{
    ...
    public function get_pasien_rajal($start_date = null, $end_date = null, $kd_pj = null, $status_bayar = null, $status_periksa = null)
    {
        $this->db->select("
            ...
            (CASE WHEN EXISTS (
                SELECT 1 FROM moizhospital_antrian_poli ap
                WHERE ap.no_rawat = r.no_rawat
                  AND DATE(ap.tgl_panggil) = CURDATE()
                LIMIT 1
            ) THEN 1 ELSE 0 END) AS sudah_dipanggil
        ", false);
        ...
        // filter tanggal
        if ($start_date)
            $this->db->where('r.tgl_registrasi >=', $start_date);
        if ($end_date)
            $this->db->where('r.tgl_registrasi <=', $end_date);

        if ($kd_pj)
            $this->db->where('r.kd_pj', $kd_pj);      // filter by kode penjamin
        if ($status_bayar)
            $this->db->where('r.status_bayar', $status_bayar);
        if ($status_periksa)
            $this->db->where('r.stts', $status_periksa);
