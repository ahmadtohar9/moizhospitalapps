<!-- ASESMEN AWAL MEDIS KULIT - COMPLETE CLEAN VERSION -->
<style>
    .section-header {
...
<div class="container-fluid">
    <form id="formKULITAssessment" autocomplete="off">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="rpk" value="" placeholder="Riwayat penyakit di keluarga...">
        </div>
            ...
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="status" value="" required
                        placeholder="Status kondisi pasien...">
                </div>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-2">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="suhu" value="" required placeholder="36.5">
                </div>
            </div>
            <div class="col-md-2">
                <div class="form-group">
            ...
            <div class="col-md-2">
                <div class="form-group">
                    <label>Nyeri</label>
                    <input type="text" class="form-control" name="nyeri" value="" placeholder="Skala nyeri...">
                </div>
            </div>
        </div>

        <!-- III. STATUS LOKALIS -->
        <div class="section-header"><i class="fa fa-image"></i> III. STATUS LOKALIS KULIT</div>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <div class="canvas-wrapper" style="height:450px;">
                        ...
                        <canvas id="kulitCanvas" width="600" height="350"></canvas>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    <label>Status Dermatologis / Lokalis</label>
                    <textarea class="form-control" name="statusderma" rows="18"
                        placeholder="Deskripsi status dermatologis dan lokalis..."></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="form-group">
            <label>Pemeriksaan Penunjang (Lab/Rad/Lainnya)</label>
            <textarea class="form-control" name="pemeriksaan" rows="3" placeholder="Hasil pemeriksaan penunjang..."
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" name="diagnosis" rows="2" required
                placeholder="Diagnosis Utama..."></textarea>
        </div>

        <div class="form-group">
            <label>Diagnosis Sekunder / Tambahan</label>
            <textarea class="form-control" name="diagnosis2" rows="2" placeholder="Diagnosis Sekunder..."></textarea>
        </div>

        <div class="form-group">
            <label>Permasalahan</label>
            <textarea class="form-control" name="permasalahan" rows="2"
                placeholder="Daftar permasalahan pasien..."></textarea>
        </div>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label>Terapi</label>
                    <textarea class="form-control" name="terapi" rows="3" placeholder="Rencana terapi..."></textarea>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label>Tindakan</label>
                    <textarea class="form-control" name="tindakan" rows="3" placeholder="Tindakan yang dilakukan..."
                        maxlength="100"></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="form-group">
            <label>Edukasi</label>
            <textarea class="form-control" name="edukasi" rows="2" placeholder="Edukasi kepada pasien..."></textarea>
        </div>
...
<div class="container-fluid" style="margin-top:30px;">
    <div class="section-header" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%);"><i
            class="fa fa-history"></i> RIWAYAT ASESMEN KULIT</div>
...
<script>
    ...
    var backgroundURL = '<?= base_url("assets/images/status_lokalis_kulit.png") ?>';

    setTimeout(function () {
        canvas = document.getElementById('kulitCanvas');
        if (!canvas) return;
    ...
    window.resetFormAndButton = function () {
        document.getElementById('formKULITAssessment').reset();
    ...
    // Form Submit
    document.getElementById('formKULITAssessment').addEventListener('submit', function (e) {
        e.preventDefault();
        const formData = new FormData(this);

        fetch('<?= base_url("AwalMedisKulitDanKelaminController/save") ?>', {
            method: 'POST',
            body: formData
        })
            .then(res => res.json())
    ...
    function loadHistory() {
        ...
        fetch('<?= base_url("AwalMedisKulitDanKelaminController/get_history") ?>?no_rkm_medis=' + noRkm + '&start_date=' + startDate + '&end_date=' + endDate)
            .then(res => res.json())
            .then(data => {
    ...
    function viewDetail(noRawat) {
        fetch('<?= base_url("AwalMedisKulitDanKelaminController/get_detail") ?>?no_rawat=' + noRawat)
            .then(res => res.json())
            .then(data => {
                if (data.status === 'success') {
                    const d = data.data;
                    ...
                    html += '<p><strong>RPK:</strong> ' + (d.rpk || '-') + '</p></div>';
                    ...
                    if (d.statusderma) {
                        ...
                        html += '<h5 style="margin:0 0 10px 0; color:#ef4444;"><i class="fa fa-image"></i> III. STATUS LOKALIS & DERMATOLOGIS</h5>';
                        html += '<p><strong>Deskripsi:</strong><br>' + d.statusderma + '</p>';
                        var imgFile = 'lokalis_' + d.no_rawat.replace(/\//g, '') + '.png';
                        var imgUrl = '<?= base_url("assets/images/lokalis_kulit/") ?>' + imgFile;
                    ...
                    typeof Swal !== "undefined" && Swal.fire({
                        title: '<span style="color:#ec4899;"><i class="fa fa-file-medical"></i> Detail Asesmen KULIT</span>',
                        html: html,
                        width: '900px',
        ...
        fetch('<?= base_url("AwalMedisKulitDanKelaminController/get_detail") ?>?no_rawat=' + noRawat)
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    const fields = ['no_rawat', 'anamnesis', 'hubungan', 'keluhan_utama', 'rps', 'rpd', 'rpo', 'rpk',
                        'kesadaran', 'gcs', 'status', 'td', 'nadi', 'rr', 'suhu', 'bb', 'nyeri',
                        'statusderma', 'pemeriksaan', 'diagnosis', 'diagnosis2',
    ...
    function printSinglePDF(noRawat) {
        window.open('<?= base_url("AwalMedisKulitDanKelaminController/print_pdf?no_rawat=") ?>' + noRawat, '_blank');
    }
    ...
    function deleteAssessmentHistory(noRawat) {
        typeof Swal !== "undefined" && Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch('<?= base_url("AwalMedisKulitDanKelaminController/delete") ?>', {
                    method: 'POST',
    ...
    (function () {
        setTimeout(function () {
            const form = document.getElementById('formKULITAssessment');
            if (form) {
