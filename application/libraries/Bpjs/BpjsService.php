    ...
    private function _exec_v2($url, $method, $payload)
    {
        ...
        if (isset($response['metaData']['code'])) {
            if ($response['metaData']['code'] == 200) {
                ...
                // Logic extraction similar to insert logic to be safe
                $sep_data = $respBody['sep'] ?? $respBody['t_sep'] ?? $respBody ?? [];

                return [
                    'status' => 'success',
                    'message' => $response['metaData']['message'] ?? 'SEP processed successfully',
                    'data' => $sep_data, // Return the extracted SEP Object
                    'raw' => $response
                ];
            } else {
                return [
