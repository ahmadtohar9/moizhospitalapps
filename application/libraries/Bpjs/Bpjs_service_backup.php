...
class Bpjs_service
{
    ...
    {
        // Unix timestamp is already "seconds since 1970-01-01 00:00:00 UTC"
    ...
    public function send_request($endpoint, $method = 'GET', $data = null, $timestamp = null)
    {
        ...
        // Ensure content type JSON for POST/PUT
        ...
        return [
            'metaData' => ['code' => $http_code, 'message' => 'Remote returned non-JSON error page'],
            'debug' => ['http_code' => $http_code, 'raw_start' => substr($raw_response, 0, 2000), 'raw_full' => (strlen($raw_response) <= 5000 ? $raw_response : null)]
        ];
    }
    ...
    public function insert_sep_v2($t_sep_data)
    {
        ...
        // --- FIX: Align with Postman ---
        // VClaim Insert SEP 2.0 explicitly requires Content-Type: text/plain
        // If we send application/json, it returns 400 Bad Request (HTML Error)
        // If we send text/plain, it works (or returns business logic error)

        $headers = array_filter($headers, function ($h) {
            return stripos($h, 'Content-Type:') === false;
        });

        // Re-index array to be safe for curl
        ...
        // Prepare body with unescaped slashes/unicode to match raw string best
        $body = json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);

        // Log untuk debugging
        log_message('debug', '[Bpjs_service][insert_sep_v2] URL: ' . $url);
        log_message('debug', '[Bpjs_service][insert_sep_v2] Payload: ' . substr($body, 0, 1000));
        log_message('debug', '[Bpjs_service][insert_sep_v2] Headers: ' . json_encode($headers));
        log_message('debug', '[Bpjs_service][insert_sep_v2] Timestamp: ' . $timestamp);
        log_message('debug', '[Bpjs_service][insert_sep_v2] Cons ID: ' . $this->cons_id);
        ...
        $options = [
            ...
            CURLOPT_TIMEOUT => 30,
            CURLOPT_FOLLOWLOCATION => true,
            CURLOPT_SSL_VERIFYPEER => false, // Nonaktifkan SSL verify untuk development
        ...
        // Handle CURL errors
        if ($curl_errno) {
            log_message('error', '[Bpjs_service][insert_sep_v2] CURL Error: ' . $curl_error);
            return [
                'status' => 'error',
        ...
        // Log raw response
        log_message('debug', '[Bpjs_service][insert_sep_v2] HTTP Code: ' . $http_code);
        log_message('debug', '[Bpjs_service][insert_sep_v2] Raw Response: ' . substr($raw_response, 0, 500));
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            ...
            log_message('error', '[Bpjs_service][insert_sep_v2] Invalid JSON response: ' . json_last_error_msg());
            return [
                'status' => 'error',
        ...
        if (isset($response['metaData']['code'])) {
            if ($response['metaData']['code'] == 200) {
                // Success
                // Response structure usually: { metaData:..., response: { sep: {...} } }
                $respBody = $response['response'] ?? [];
                $sep_data = $respBody['sep'] ?? $respBody['t_sep'] ?? $respBody ?? [];

                return [
                    'status' => 'success',
                    'message' => $response['metaData']['message'] ?? 'SEP created successfully',
                    'data' => $sep_data,
