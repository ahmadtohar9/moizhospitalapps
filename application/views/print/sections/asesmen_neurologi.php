...
<div class="print-section">
    ...
    <?php if (!empty($d->neurologi->anamnesis)): ?>
    <div style="margin: 10px 0;">
        <strong>Anamnesis:</strong><br>
        <?= nl2br($d->neurologi->anamnesis) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->neurologi->pemeriksaan_fisik)): ?>
    <div style="margin: 10px 0;">
        <strong>Pemeriksaan Fisik:</strong><br>
        <?= nl2br($d->neurologi->pemeriksaan_fisik) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->neurologi->pemeriksaan_penunjang)): ?>
    <div style="margin: 10px 0;">
        <strong>Pemeriksaan Penunjang:</strong><br>
        <?= nl2br($d->neurologi->pemeriksaan_penunjang) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->neurologi->diagnosis)): ?>
    <div style="margin: 10px 0;">
        <strong>Diagnosis:</strong><br>
        <?= nl2br($d->neurologi->diagnosis) ?>
    </div>
    <?php endif; ?>
    
    <?php if (!empty($d->neurologi->tatalaksana)): ?>
    <div style="margin: 10px 0;">
        <strong>Tatalaksana:</strong><br>
        <?= nl2br($d->neurologi->tatalaksana) ?>
    </div>
    <?php endif; ?>
    ...
    $lokalis_path = FCPATH . 'assets/images/lokalis_neurologi/lokalis_' . $clean_no_rawat . '.png';
    if (file_exists($lokalis_path)):
    ?>
    <div style="margin: 15px 0;">
        <strong>Gambar Lokalis:</strong><br>
        <img src="<?= base_url('assets/images/lokalis_neurologi/lokalis_' . $clean_no_rawat . '.png') ?>" 
