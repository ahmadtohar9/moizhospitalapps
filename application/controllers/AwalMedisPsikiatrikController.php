...
class AwalMedisPsikiatrikController extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('AwalMedisPsikiatrikModel');
    ...
    public function index()
    {
        ...
        $this->load->view('rekammedis/dokter/awalMedisPsikiatrik_view', $data);
    }
    ...
    public function get_history()
    {
        ...
        $history = $this->AwalMedisPsikiatrikModel->get_history($no_rkm_medis, $start_date, $end_date);
    ...
    public function get_detail()
    {
        ...
        $detail = $this->AwalMedisPsikiatrikModel->get_by_no_rawat($no_rawat);
        if ($detail) {
    ...
    public function save()
    {
        ...
        try {
            ...
            // Image processing removed as per request

            // Whitelist: only save columns that exist in penilaian_medis_ralan_psikiatrik table
            $allowed = [
                'no_rawat',
                'tanggal',
                'kd_dokter',
            ...
            if ($this->AwalMedisPsikiatrikModel->exists($post['no_rawat'])) {
                $this->AwalMedisPsikiatrikModel->update($post['no_rawat'], $post);
            } else {
                $this->AwalMedisPsikiatrikModel->insert($post);
            }
    ...
    public function delete()
    {
        ...
        if ($this->AwalMedisPsikiatrikModel->delete($no_rawat)) {
    ...
    public function print_pdf()
    {
        ...
        $data['asesment'] = $this->AwalMedisPsikiatrikModel->get_by_no_rawat($no_rawat);

        $clean_no_rawat = str_replace('/', '', $no_rawat);

        // No image needed for Psikiatrik
        $data['lokalis_path'] = null;

        $this->load->model('SettingModel');
        $data['setting'] = $this->SettingModel->get_setting();

        $html = $this->load->view('rekammedis/dokter/pdf_awal_medis_psikiatrik', $data, true);
        ...
        $mpdf->WriteHTML($html);
        $mpdf->Output('Asesmen_Psikiatrik_' . $clean_no_rawat . '.pdf', 'I');
    }
}
