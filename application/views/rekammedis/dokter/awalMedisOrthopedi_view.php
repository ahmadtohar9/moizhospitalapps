...
<form id="formAwalMedisOrthopedi">
    ...
    <div class="row mb-2">
        <div class="col-md-12">
            <h3 style="margin-top:0;"><i class="fa fa-wheelchair"></i> Asesmen Awal Medis Orthopedi</h3>
        </div>
    ...
    <div class="modal-dialog modal-lg" style="width: 90%;">
        <div class="modal-content" style="border-radius: 5px;">
            <div class="modal-header bg-blue" style="border-top-left-radius: 5px; border-top-right-radius: 5px;">
                ...
                <h4 class="modal-title"><i class="fa fa-file-text-o"></i> Detail Asesmen Orthopedi</h4>
            </div>
...
<script>
    ...
    $saved_file = FCPATH . 'assets/images/lokalis_orthopedi/lokalis_' . $clean_no_rawat . '.png';
    $saved_url = base_url('assets/images/lokalis_orthopedi/lokalis_' . $clean_no_rawat . '.png');
    $has_image = file_exists($saved_file) ? 'true' : 'false';
    ?>
    var existingImageURL = <?= $has_image == 'true' ? "'$saved_url?t=" . time() . "'" : "''" ?>;
    var backgroundURL = '<?= base_url("assets/images/human_body_anatomy_orthopedi.png") ?>';
    ...
    function simpanOrtho() {
        ...
        $.ajax({
            url: "<?= base_url('AwalMedisOrthopediController/save') ?>",
            type: "POST",
            data: $('#formAwalMedisOrthopedi').serialize(),
            dataType: "json",
            beforeSend: function () {
    ...
    function hapusOrtho() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post("<?= base_url('AwalMedisOrthopediController/delete') ?>",
                    { no_rawat: $('#ortho_no_rawat').val() },
                    function (res) {
                        if (res.status == 'success') {
                            Swal.fire('Terhapus', res.message, 'success').then(() => {
                                $('#formAwalMedisOrthopedi')[0].reset();
                                clearCanvas();
    ...
    function cetakPdfOrtho() {
        ...
        if (!nr) return;
        window.open('<?= base_url("AwalMedisOrthopediController/print_pdf?no_rawat=") ?>' + nr, '_blank');
    }
