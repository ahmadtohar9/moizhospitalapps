...
class AwalMedisAnakController extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('AwalMedisAnakModel');
    ...
    public function index()
    {
        ...
        $this->load->view('rekammedis/dokter/awalMedisAnak_view', $data);
    }
    ...
    public function get_history()
    {
        ...
        $history = $this->AwalMedisAnakModel->get_history($no_rkm_medis, $start_date, $end_date);
    ...
    public function get_detail()
    {
        ...
        $detail = $this->AwalMedisAnakModel->get_by_no_rawat($no_rawat);
        if ($detail) {
    ...
    public function save()
    {
        ...
        try {
            ...
            if (!empty($post['lokalis_image'])) {
                ...
                if ($data) {
                    $dir = FCPATH . 'assets/images/lokalis_anak/';
                    if (!is_dir($dir))
                        mkdir($dir, 0777, true);
            ...
            if ($this->AwalMedisAnakModel->exists($post['no_rawat'])) {
                $this->AwalMedisAnakModel->update($post['no_rawat'], $post);
            } else {
                $this->AwalMedisAnakModel->insert($post);
            }
    ...
    public function delete()
    {
        ...
        $clean = str_replace('/', '', $no_rawat);
        $path = FCPATH . 'assets/images/lokalis_anak/lokalis_' . $clean . '.png';
        if (file_exists($path))
            unlink($path);

        if ($this->AwalMedisAnakModel->delete($no_rawat)) {
    ...
    public function print_pdf()
    {
        ...
        $data['asesment'] = $this->AwalMedisAnakModel->get_by_no_rawat($no_rawat);
        ...
        $html = $this->load->view('rekammedis/dokter/pdf_awal_medis_anak', $data, true);
        ...
        $mpdf->WriteHTML($html);
        $mpdf->Output('Asesmen_Anak_' . $clean_no_rawat . '.pdf', 'I');
    }
}
