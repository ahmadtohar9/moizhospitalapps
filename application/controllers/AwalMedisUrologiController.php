...
class AwalMedisUrologiController extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('AwalMedisUrologiModel');
    ...
    public function index()
    {
        ...
        $this->load->view('rekammedis/dokter/awalMedisUrologi_view', $data);
    }
    ...
    public function get_history()
    {
        ...
        $history = $this->AwalMedisUrologiModel->get_history($no_rkm_medis, $start_date, $end_date);
    ...
    public function get_detail()
    {
        ...
        $detail = $this->AwalMedisUrologiModel->get_by_no_rawat($no_rawat);
        if ($detail) {
    ...
    public function save()
    {
        ...
        try {
            ...
            if (!empty($post['lokalis_image'])) {
                ...
                if ($data) {
                    $dir = FCPATH . 'assets/images/lokalis_urologi/';
                    if (!is_dir($dir))
                        mkdir($dir, 0777, true);
            ...
            // Whitelist: only save columns that exist in penilaian_medis_ralan_urologi table
            $allowed = [
                'no_rawat',
                'tanggal',
                'kd_dokter',
            ...
            if ($this->AwalMedisUrologiModel->exists($post['no_rawat'])) {
                $this->AwalMedisUrologiModel->update($post['no_rawat'], $post);
            } else {
                $this->AwalMedisUrologiModel->insert($post);
            }
    ...
    public function delete()
    {
        ...
        $clean = str_replace('/', '', $no_rawat);
        $path = FCPATH . 'assets/images/lokalis_urologi/lokalis_' . $clean . '.png';
        if (file_exists($path))
            unlink($path);

        if ($this->AwalMedisUrologiModel->delete($no_rawat)) {
    ...
    public function print_pdf()
    {
        ...
        $data['asesment'] = $this->AwalMedisUrologiModel->get_by_no_rawat($no_rawat);

        $clean_no_rawat = str_replace('/', '', $no_rawat);
        $lokalis_path = FCPATH . 'assets/images/lokalis_urologi/lokalis_' . $clean_no_rawat . '.png';
        ...
        $html = $this->load->view('rekammedis/dokter/pdf_awal_medis_urologi', $data, true);
        ...
        $mpdf->WriteHTML($html);
        $mpdf->Output('Asesmen_Urologi_' . $clean_no_rawat . '.pdf', 'I');
    }
}
